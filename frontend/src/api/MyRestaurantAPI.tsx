...
export const useUpdateMyRestaurant = () => {
  ...
  // function to update the restaurant data and we add promise to return the restaurant data after the update
  const updateMyRestaurantRequest = async (
    restaurantFormData: FormData
  ): Promise<Restaurant> => {
    const accessToken = await getAccessTokenSilently();

    // fetching the API to update the restaurant
    const response = await fetch(`${API_BASE_URL}/api/my/restaurant`, {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${accessToken}`,
  ...
  // useMutation hook is used to make the API request to update the restaurant data
  const {
